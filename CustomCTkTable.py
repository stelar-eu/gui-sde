...
from CustomCTkCheckbox import CustomCTkCheckBox
import copy

from customtkinter import CTkImage, ThemeManager
...
class CTkTable(customtkinter.CTkFrame):
    ...
    def __init__(
        ...
        self.binded_objects = []
        self.cell_height = round(self.height / self.rows, 0)
        self.cell_width = round(self.width / self.columns, 0)
        ...
        self.inside_frame = customtkinter.CTkFrame(self, border_width=0, fg_color="transparent", bg_color="transparent") #TODO: erase bg_color
        super().configure(border_color=self.border_color, border_width=self.border_width, corner_radius=self.corner)

        self.inside_frame.grid(row=0, column=0, sticky="nsew")
    ...
    def draw_table(self, **kwargs):
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                self.inside_frame.grid_rowconfigure(i, weight=0)
                self.inside_frame.grid_columnconfigure(j, weight=0)
                ...
                if self.write:
                    ...
                    #     args["anchor"] = self.anchor #TODO: if using button, then anchor is req
                    ...
                    if "width" in args:
                        del args["width"]
                    self.frame[i, j] = CustomCTkCheckBox(self.inside_frame,
                                                        height=self.cell_height,
                                                        width=self.cell_width,
                                                        checkbox_width=self.cell_width,
                                                        checkbox_height=self.cell_height,
                    ...
                    if hover_modify:
                        self.dynamic_hover(self.frame[i, j], i, j, fg)
    ...
    def dynamic_hover(self, frame, i, j, fg=customtkinter.ThemeManager.theme["CTkFrame"]["fg_color"]):
        """ internal function to change corner cell colors """
        self.corner_buttons[i, j] = frame
        #fg = self.data[i, j]["args"]["fg_color"]
        #TODO: uncomment fg, remove fg from args
    ...
    def configure(self, **kwargs):
        ...
        if "bg_color" in kwargs:
            super().configure(bg_color="transparent")#kwargs["bg_color"])
